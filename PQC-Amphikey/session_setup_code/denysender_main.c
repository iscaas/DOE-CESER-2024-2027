...
#include "api.h"         // Ascon-128 NIST LWCA one-shot API
...
int main() {
    ...
    for (int i = 0; i < BENCHMARK_ITERATIONS; ++i) {
        ...
        clock_gettime(CLOCK_MONOTONIC, &start_time); start_cycles = rdtsc();
        crypto_auth_hmacsha256(prk, k_auth, sizeof(k_auth), zero);
        /* expand with info="ksh" */
    ...
    // --- BENCHMARK Ascon-128 AEAD Encrypt ---
    printf("\n--- Benchmarking Ascon-128 AEAD Encrypt ---\n");
    /* Use meter data representative payload (126 bytes) */
    ...
    unsigned long long ct_ascon_len = 0;
    total_ns = 0; total_cycles = 0;
    for (int i = 0; i < BENCHMARK_ITERATIONS; ++i) {
        unsigned long long tmp_len = 0;
        clock_gettime(CLOCK_MONOTONIC, &start_time); start_cycles = rdtsc();
        crypto_aead_encrypt(ct_ascon, &tmp_len,
        ...
        total_ns += (end_time.tv_sec - start_time.tv_sec) * 1000000000LL +
                    (end_time.tv_nsec - start_time.tv_nsec);
        if (end_cycles > start_cycles) total_cycles += (end_cycles - start_cycles);
        ct_ascon_len = tmp_len;
    }
    printf("  Plaintext: %zu B  →  Ciphertext+tag: %llu B\n", meter_len, ct_ascon_len);
    printf("Average Time:   %lld ns\n", total_ns / BENCHMARK_ITERATIONS);
    printf("Average Cycles: %llu\n",    total_cycles / BENCHMARK_ITERATIONS);
    printf("------------------------------------\n");
    free(ct_ascon);
