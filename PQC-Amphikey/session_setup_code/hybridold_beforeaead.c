...
int read_hex_key_from_file(const char* filename, unsigned char* bin_buffer, size_t bin_buffer_len, size_t* actual_bin_len) {
    ...
    char* hex_string_buf = (char*)malloc(bin_buffer_len * 2 + 3); // Max hex chars + newline + null
    if (!hex_string_buf) {
...
int main() {
    ...
    if (read_binary_data_from_file("server_mlkem_pk.key", pks_ml_kem, sizeof(pks_ml_kem), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(pks_ml_kem)) { fprintf(stderr, "Load server_mlkem_pk.key failed.\n"); return 1; }
    if (read_binary_data_from_file("server_mlkem_sk.key", sks_ml_kem, sizeof(sks_ml_kem), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(sks_ml_kem)) { fprintf(stderr, "Load server_mlkem_sk.key failed.\n"); return 1; }
    if (read_hex_key_from_file("server_x25519_pk.hex", pks_x25519, sizeof(pks_x25519), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(pks_x25519)) { fprintf(stderr, "Load server_x25519_pk.hex failed.\n"); return 1; }
    if (read_hex_key_from_file("server_x25519_sk.hex", sks_x25519, sizeof(sks_x25519), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(sks_x25519)) { fprintf(stderr, "Load server_x25519_sk.hex failed.\n"); return 1; }
    if (read_binary_data_from_file("server_raccoon_pk.key", pks_rac, sizeof(pks_rac), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(pks_rac)) { fprintf(stderr, "Load server_raccoon_pk.key failed.\n"); return 1; }
    if (read_binary_data_from_file("server_raccoon_sk.key", sks_rac, sizeof(sks_rac), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(sks_rac)) { fprintf(stderr, "Load server_raccoon_sk.key failed.\n"); return 1; }
    printf("Sender's (Server's) keys loaded successfully.\n");
    print_hex("Pks_ml_kem (loaded)", pks_ml_kem, sizeof(pks_ml_kem));
    print_hex("Pks_x25519 (loaded)", pks_x25519, sizeof(pks_x25519));
    print_hex("Pks_rac (loaded)", pks_rac, sizeof(pks_rac));
    printf("\n");

    // --- Load Receiver's (Client's) Public Keys from Files ---
    printf("Loading Receiver's (Client's) public keys from files...\n");
    if (read_binary_data_from_file("client_mlkem_pk.key", pkr_ml_kem, sizeof(pkr_ml_kem), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(pkr_ml_kem)) { fprintf(stderr, "Load client_mlkem_pk.key failed.\n"); return 1; }
    if (read_hex_key_from_file("client_x25519_pk.hex", pkr_x25519, sizeof(pkr_x25519), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(pkr_x25519)) { fprintf(stderr, "Load client_x25519_pk.hex failed.\n"); return 1; }
    if (read_binary_data_from_file("client_raccoon_pk.key", pkr_rac, sizeof(pkr_rac), &bytes_read_actual) != 0 || bytes_read_actual != sizeof(pkr_rac)) { fprintf(stderr, "Load client_raccoon_pk.key failed.\n"); return 1; }
    printf("Receiver's (Client's) public keys loaded successfully.\n");
    print_hex("Pkr_ml_kem (loaded)", pkr_ml_kem, sizeof(pkr_ml_kem));
    print_hex("Pkr_x25519 (loaded)", pkr_x25519, sizeof(pkr_x25519));
    print_hex("Pkr_rac (loaded)", pkr_rac, sizeof(pkr_rac));
    printf("\n");
    ...
    write_binary_data_to_file("kem_ciphertext_c1.dat", c1, sizeof(c1)); // Output for receiver
    ...
    size_t c_len = sizeof(c1) + sizeof(pks_x25519); // c_data = c1 || Pks_x25519
    ...
    write_binary_data_to_file("combined_c_for_signature.dat", c_data, c_len); // Output for receiver

    unsigned char sig[CRYPTO_BYTES]; // Raccoon's signature size
    ...
    write_binary_data_to_file("signature_on_c.sig", sig, CRYPTO_BYTES); // Output for receiver
    ...
    // Receiver's secret keys were not used/loaded in this sender program, so no need to zero them here.

    printf("\nHybrid protocol sender operations complete. Transmitted data saved to files.\n");
    return 0;
}
