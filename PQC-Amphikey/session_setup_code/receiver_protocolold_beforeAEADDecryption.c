...
// --- Main Program ---
int main() {
    printf("--- RECEIVER PROTOCOL OPERATIONS (Loading All Keys) ---\n\n");

    // 1. Initialize Randomness & Libsodium
    unsigned char entropy_input[48];
    for (int i = 0; i < 48; i++) entropy_input[i] = (unsigned char)('R' + i + 20); // Different dummy entropy
    ...
    if (read_binary_data_from_file("client_mlkem_sk.key", skr_ml_kem, sizeof(skr_ml_kem), &bytes_read_actual) != 0 ) { fprintf(stderr,"Load client_mlkem_sk.key failed\n"); return 1;}
    ...
    if (read_hex_key_from_file("client_x25519_sk.hex", skr_x25519, sizeof(skr_x25519), &bytes_read_actual) != 0 ) { fprintf(stderr,"Load client_x25519_sk.hex failed\n"); return 1;}
    ...
    if (read_binary_data_from_file("client_raccoon_sk.key", skr_rac, sizeof(skr_rac), &bytes_read_actual) != 0 ) { fprintf(stderr,"Load client_raccoon_sk.key failed\n"); return 1;}
    ...
    if (read_binary_data_from_file("server_mlkem_pk.key", pks_ml_kem_from_sender, sizeof(pks_ml_kem_from_sender), &bytes_read_actual) != 0 ) { fprintf(stderr,"Load server_mlkem_pk.key failed\n"); return 1;}
    if (read_hex_key_from_file("server_x25519_pk.hex", pks_x25519_from_sender, sizeof(pks_x25519_from_sender), &bytes_read_actual) != 0 ) { fprintf(stderr,"Load server_x25519_pk.hex failed\n"); return 1;}
    if (read_binary_data_from_file("server_raccoon_pk.key", pks_rac_from_sender, sizeof(pks_rac_from_sender), &bytes_read_actual) != 0 ) { fprintf(stderr,"Load server_raccoon_pk.key failed\n"); return 1;}
    printf("Sender's (Server's) public keys loaded.\n");
    ...
    // --- Load Transmitted Data (c1, sig) from Sender (hybrid.c output) ---
    printf("Loading transmitted data (c1, sig) from hybrid.c output files...\n");
    if (read_binary_data_from_file("kem_ciphertext_c1.dat", c1_from_sender, sizeof(c1_from_sender), &bytes_read_actual) != 0 ) { fprintf(stderr, "Failed to load kem_ciphertext_c1.dat\n"); return 1; }
    if (read_binary_data_from_file("signature_on_c.sig", sig_from_sender, sizeof(sig_from_sender), &bytes_read_actual) != 0 ) { fprintf(stderr, "Failed to load signature_on_c.sig\n"); return 1; }
    printf("Transmitted data (c1, sig) loaded.\n");
    ...
    // 2. Verify Sender's Signature using loaded pks_rac_from_sender
    ...
    // 4. X25519 Shared Secret by Receiver using loaded skr_x25519 and loaded pks_x25519_from_sender
    ...
    // Pks_bytes_from_sender uses sender's loaded public keys
    size_t pks_bytes_from_sender_len = sizeof(pks_ml_kem_from_sender) + sizeof(pks_x25519_from_sender) + sizeof(pks_rac_from_sender);
